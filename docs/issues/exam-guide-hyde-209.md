...
`business intelligence` → `Amazon QuickSight`）を追加し、`augmentTermsWithCatalog` は
**クエリが概念語（フレーズ）を含む**とき（前方向 `q.includes(concept)` のみ）対応サービスを
「クエリ語」として catalog 照合に載せる（catalog に無いサービスは加えない）。純粋・テスト可能・
低リスク。v2 レビューで、逆方向（概念語がクエリ語を含む）を落として過剰発火を締めた。短い/一般的な
クエリ語（例 `b` / `business` / `ビジネス`）が概念語の部分文字列というだけで QuickSight を
注入する誤動作を防ぐためで、`BIツール` や `ダッシュボード` を含むクエリは前方向で引き続き発火する。
