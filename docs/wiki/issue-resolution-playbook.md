...
- 出典/参照: `.kiro/agents/github-issue-resolver.md` / `docs/wiki/efficiency-log.md` / issue #32 #109 #137 #138 #161〜#169 #184 #194 #197 #200 / PR #113 #114 #139 #157 #160 #175〜#185 #205 #214
...
### A-16: 要人間対応 issue の中に隠れた「エージェント可」サブタスクを切り出さず、issue 全体を寝かせていた

- **何が起きたか**: issue #197（多言語対応）は「翻訳文の作成には人間の製品判断が要る」ため要人間対応と分類されていた。ところが `docs/action-required/issue-197-multilanguage-support.md` は、翻訳文とは独立に **i18n コアの N ロケール一般化（翻訳非依存）を『エージェント可』と明記**していた。にもかかわらず issue 全体が要人間対応の一枚岩として扱われたため、この切り出せるサブタスク（`js/i18n.js` の `SUPPORTED_LOCALES` / `getLocalizedUrl` などを 2 値固定から N ロケールへ一般化）が分離・着手されないまま長く放置された。
- **根本原因**: 「要人間対応」というマーカーを **issue 単位の二値**として扱い、doc 自身がサブタスク粒度で付けていた『エージェント可』の分類を無視していた。要人間対応＝何もしない、という短絡。
- **恒久対策**: issue が `要人間対応` でも、`docs/action-required/` の該当 doc が**サブタスク単位で『エージェント可』と分類している項目**は、通常の `TRIAGE` として扱ってクライアント側で前進させる。その分は `Closes` ではなく **`Refs #n` の部分対応 PR** として出し、残る要人間対応（ここでは各言語の翻訳文）は `docs/action-required/` に残置してクローズしない。issue のクローズ可否とサブタスクの出荷可否は別問題として切り分ける。
- **補足（マージ済み PR が `(#n)` 参照だけで issue を open のまま残す）**: 要望を満たすマージ済み PR が、本文に `Closes #n` を書かずタイトルで `(#n)` と**参照しただけ**だと、GitHub の自動クローズが働かず issue は open のまま滞留する（実例: #200 に対する PR #205）。棚卸しで `TRIAGE`（マージ済み関連 PR あり・issue open）と出た issue は、そのマージ済み成果が要望を満たしていることを確認したうえで、**説明コメント付きでクローズ**する。これは A-15 の「SKIP 済み issue への再コメント禁止」とは**別カテゴリ**の正当な状態整理であり、混同しない（A-15 は未対応で寝かせる issue への重複コメントの禁止、A-16 のこの補足は完了済みの issue を閉じる操作）。
- **教訓（一般化）**: 分類のマーカーは、それが付いている**粒度**を確認して適用する。issue に貼られたラベルより、doc がサブタスク単位で書いた分類の方が細かいなら、細かい方に従って前進できる分を切り出す。「要人間対応だから全部保留」は、doc がわざわざ切り分けてくれた作業を捨てているのと同じ。
...
- 2026-09-12: 落とし穴 **A-16（要人間対応 issue の中に隠れた「エージェント可」サブタスクを切り出さず放置していた）** を追記。恒久対策として、要人間対応でも doc がサブタスク単位で『エージェント可』と分類した項目は通常の `TRIAGE` として前進させ `Refs #n` の部分対応 PR で出す運用を明文化（#197 の i18n コア一般化を実践）。あわせて、マージ済み PR が `Closes #n` ではなく `(#n)` 参照だけで issue を open のまま残すケース（#200 に対する PR #205）は、要望充足を確認のうえ説明コメント付きでクローズする（A-15 の SKIP 再コメント禁止とは別カテゴリ）ことを A-16 の補足として記載。出典: issue #197 #200 / PR #205 #214 / `docs/action-required/issue-197-multilanguage-support.md`。
