...
- 出典/参照: `.kiro/agents/github-issue-resolver.md` / `docs/wiki/efficiency-log.md` / issue #32 #109 #137 #138 #161〜#169 #184 #190 #194 / PR #113 #114 #139 #157 #160 #175〜#185 #195 #203 #206
...
### A-16: `issue-triage.mjs` をバックグラウンド実行したまま観測シェルがタイムアウトで殺され、空の出力ファイルが残った

- **何が起きたか**: トリアージ（`scripts/issue-triage.mjs`）はオープン PR 監査を含むため 1〜3 分かかる。これをリポジトリ内で `... &` のバックグラウンドジョブとして起動し、同じシェルで出力ファイルをポーリングしていたところ、**観測している側のシェル呼び出しがツールのタイムアウトで打ち切られた**。バックグラウンドジョブは親シェルの終了に巻き込まれて死に、**書きかけ（実質空）の出力ファイルだけが残った**。中身が空なのを「issue が 0 件」と誤読しかける危険もあった（本セッションで実際に発生）。
- **根本原因**: (a) ジョブを親シェルにアタッチしたまま（`&` だけ）起動したため、親シェルが殺されると子も死ぬ。(b) 出力先を**リポジトリ内**に置いたため、途中経過の空ファイルがワークツリーに紛れ込んだ。(c) 「1〜3 分かかる」という所要時間は playbook に書いてあったが、「**短時間タイムアウトのシェルで待つな**」という運用が徹底されていなかった。
- **恒久対策**: (1) トリアージは**親シェルから切り離して実行する**。bash では `nohup node scripts/issue-triage.mjs > /var/tmp/asn-scratch/triage.out 2>&1 &`（または `setsid ...`）のように起動し、出力は**リポジトリ外**（例 `/var/tmp/asn-scratch/`）へ書く。(2) 起動シェルはすぐ戻し、**別の短いシェル呼び出しでファイルの完了を確認**する（末尾に完了マーカー行が出たか、あるいはファイルサイズが増えなくなったかをポーリングする）。(3) **短時間タイムアウトのシェルで完了を待たない**（`sleep` して同一呼び出しでブロックしない）。(4) 出力ファイルが空 / 途中で切れている場合は「issue 0 件」と即断せず、再実行して完走を確認する。
- **教訓（一般化）**: 「所要時間が分かっている長時間ジョブ」は、**観測プロセスの寿命とジョブの寿命を分離**する（detached 起動 + リポジトリ外へのファイル出力 + 別シェルでのポーリング）。`&` だけでは分離にならない。空の出力を成功と誤認しないため、**完了マーカーで完走を判定**する。

### A-17: 長期 SKIP の issue が、最新スキップマーカーより前の「未回答の人間依頼」を隠していた

- **何が起きたか**: issue #32（グローバルリーダーボード）は `agent:skipped`（`要人間対応` = AWS バックエンドが必要）として長らく除外されていた。ところが、その issue には**最新のスキップマーカーより前**に付いていた人間からの具体的な依頼（DynamoDB を使う場合の**コスト試算**を出してほしい）があり、それが**一度も実質的に回答されていなかった**。マーカー再評価は「マーカー付与後に新しい人間更新があるか（`updatedAfterMarker` / `SKIP` / `RECHECK`）」だけを見るため、**マーカーより前の未回答依頼は永久に SKIP の陰に隠れる**構造だった。
- **根本原因**: SKIP 判定は「issue 全体を AWS 必須だから見送る」という粒度でしか issue を扱っておらず、**1 つの issue の中に「AWS 操作が必要な部分」と「分析・ドキュメントだけで前進できる部分（AWS 操作不要）」が混在しうる**ことを考慮していなかった。「マーカーより前だから対応済み」という暗黙の思い込みも誤り（マーカーは"判断時点"を記録するだけで、過去の依頼に答えた証拠ではない）。
- **恒久対策**: (1) SKIP 済み issue を再びスキップする前に、**過去の人間コメント（マーカー前も含む）が実質的に回答されたことがあるか**を確認する。「マーカーより前」＝「対応済み」ではない。(2) 依頼を **「AWS 操作が本質的に必要」（＝ SKIP のまま。ただし `docs/action-required/` に成果物を残す）** と **「分析・ドキュメントだけで前進でき、AWS 操作を伴わない」（＝ SKIP のままにせず**成果物として届ける**）** に切り分ける。#32 のコスト試算は後者であり、AWS リソースを一切操作せずに料金モデルの試算ドキュメント（`docs/action-required/`）として提供できた。(3) この切り分けは「スキップしてよい / いけない」の判断基準に反映する（下表）。
- **教訓（一般化）**: `要人間対応` ラベルは issue を**丸ごと**除外する強い信号に見えるが、issue には往々にして**AWS 操作を伴わない前進可能部分**（試算・設計ドキュメント・クライアント側の下準備）が含まれる。SKIP は「AWS 操作が必要な残余」に対してのみ有効で、**分析・ドキュメント成果物はマーカーの有無に関わらず届ける**。

### A-18: 試験ガイドが PDF から Web ページへ移行した（タスクステートメント検証の正典を切り替える）

- **何が起きたか**: 一部試験の公式試験ガイドが**PDF から Web ページ（HTML）へ移行**した（issue #190 / PR #195 のフォローアップで判明）。従来はガイド PDF を正典としてタスクステートメントの文言（`js/data/*.js` の `jpTitle` / `title` / `description[]`）を突き合わせていたが、PDF が 404 / 陳腐化する、あるいは Web 版と内容がずれるケースが出てきた。
- **根本原因**: タスクステートメントの正典を「配布形態（PDF）」に固定していた。AWS は公式ガイドの一次配布を Web ページへ寄せてきており、PDF はスナップショットまたは廃止対象になりうる。
- **恒久対策**: **タスクステートメント検証の正典は Web 版試験ガイドを既定とする**（`aws.amazon.com` 上の各試験の Exam Guide ページ）。PDF しか無い試験は PDF を使うが、Web 版が存在する場合は Web 版を優先し、`js/data/*.js` の文言・ドメイン / タスク構成は Web 版に合わせて検証する。リンク死活の確認（`scripts/check-resource-links.mjs`）でも PDF URL が 403 / 消滅していれば Web 版へ差し替える（既に MLA-C01 / AIP-C01 のガイド PDF 4 URL の 403 → HTML 差し替えの前例あり。efficiency-log 2026-09-06 参照）。
- **教訓（一般化）**: 一次情報の**配布形態が変わる**ことを前提に、「どの形態を正典とみなすか」を明示的に固定し、移行が起きたら go-forward ルールとして書き戻す。ネットワーク不可の環境では Web 版を取得できないため、その場合は「Web 版を正典とするが今回は未取得」と明記して前進部分を切り離す（A-3 / A-14 と同じ考え方）。
...
| `SKIP` | マーカー付与後に更新なし | 再調査しない。スキップ一覧に載せる。**ラベル・コメントは残すが、新規スキップコメントは付けない（再コメント禁止）**。`agentSkipCommentCount` / `reSkipAdvice` で蓄積を確認する（A-15）。**ただし再スキップ前に、マーカー前も含む過去の人間依頼が実質回答済みかを確認し、AWS 操作不要な分析・ドキュメント依頼は SKIP にせず届ける（A-17）** |
...
- **いけない**: (a) オープン PR があるだけの理由、(b) end-to-end 検証ができないだけの理由、(c) 外部事実を確認できないだけの理由、(d) **`要人間対応` ラベルが付いているだけの理由**。ラベルは issue 全体を丸ごと除外する信号に見えるが、**同じ issue 内に AWS 操作を伴わない前進可能部分（コスト試算・設計ドキュメント・クライアント側の下準備）が混在しうる**（A-17）。
- **SKIP 済み issue を再スキップする前の必須確認（A-17）**: (i) 過去の人間コメント（**最新スキップマーカーより前のものも含む**）が**実質的に回答されたことがあるか**を確認する。「マーカーより前」＝「対応済み」ではない（マーカーは"判断時点"を記録するだけ）。(ii) 未回答の依頼を **「AWS 操作が本質的に必要」（SKIP 継続。`docs/action-required/` に成果物を残す）** と **「分析・ドキュメントだけで前進でき AWS 操作を伴わない」（SKIP にせず成果物として届ける。例: #32 の DynamoDB コスト試算）** に切り分ける。
...
- 2026-09-12: 落とし穴 **A-16（`issue-triage.mjs` をバックグラウンド実行したまま観測シェルがタイムアウトで殺され空の出力ファイルが残った）**、**A-17（長期 SKIP の issue が最新マーカーより前の未回答の人間依頼を隠していた）**、**A-18（試験ガイドが PDF から Web ページへ移行 → タスクステートメント検証の正典を Web 版に切り替える）** を追記。あわせて「スキップしてよい / いけない」に (d) `要人間対応` ラベルだけを理由に丸ごとスキップしないことと、再スキップ前の必須確認（マーカー前の未回答依頼の確認 / AWS 操作要否での切り分け）を追加し、「棚卸しの分岐」判定表の `SKIP` 行に A-17 の注意を追記。プロンプト（`.kiro/agents/github-issue-resolver.md`）手順 1 のトリアージ実行注記に「detached（`nohup`/`setsid`）+ リポジトリ外へのファイル出力 + 別シェルでのポーリング」を明文化（A-16 の恒久ゲート化）。出典: 本セッションのレトロスペクティブ / issue #32（マーカー前の DynamoDB コスト試算依頼が未回答だった）/ issue #190・PR #195（試験ガイドの PDF → Web 移行フォローアップ）/ PR #203・PR #206（本バッチの issue 対応 PR）。
