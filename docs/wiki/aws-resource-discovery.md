...
ネットワーク可能な環境で実ページを確認し、次のような手掛かりからレベルを判定します。
...
- リポジトリ内で判断可能な範囲（`note` / `noteEn` やタイトルに `Introductory/Intermediate/Advanced (Level NNN)`、Black Belt の `基礎編` / `Deep Dive` など明確な手掛かりがあるもの）に限り、オフラインでも控えめに付与できます（FEAT-001 で ANS の一部アイテムに実証済み）。
- 網羅的なレベル確定は実ページ確認が必要なため、後述のとおりネットワーク可能な環境（Kiro automations / `exam-content-maintainer` エージェント）で実施します。本サンドボックス（INTEGRATIONS_ONLY）では外部アクセスができないため対象外です。
