...
- `index.html` の `FAQPage` 構造化データに、実機能に即した Q&A を 2 件追加:
  - 「APIキーなしで使えますか？」→ 静的な問題バンクを使う学習コンテンツ（本番形式の模擬試験を含む）は APIキー不要。AI がその場で問題を生成するクイズモードや AIチューターチャットのみ APIキーが必要、という実態を回答。※「APIキー不要」の対象を特定機能に限定・矮小化せず、本番形式の試験でも APIキーなしで学習できることを正しく記載（PR #132 の指摘を反映）。
  - 「フィードバックの送信にGitHubアカウントは必要ですか？」→ GitHub Issue として投稿する場合はサインインが必要である一方、コピー経路や公開 Issue 一覧の閲覧はアカウント不要、という実態を回答。※「アカウント不要」と断定せず、GitHub 投稿にはサインインが必要である点を明示（PR #132 の指摘を反映）。
