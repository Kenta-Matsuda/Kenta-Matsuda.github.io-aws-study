...
> ⚠️ **料金は変動します。** 下記の単価は東京リージョン（ap-northeast-1）のオンデマンド料金の「代表値」として引用した概算です（比較のため、より安価な米国東部・バージニア北部リージョン（us-east-1）の代表値も併記します）。いずれも公開されている一般的な料金体系の知識から記述したものであり、**AWS 料金 API / コンソールへの照会は一切行っていません。必ず公式の [AWS DynamoDB 料金ページ](https://aws.amazon.com/jp/dynamodb/pricing/) および [AWS Pricing Calculator](https://calculator.aws/) で人間が最新の確定値を再確認してください。** 為替・値下げ・リージョン差・課金体系の変更により結果は変わります。
...
- **月間アクティブユーザー（MAU）**: 総登録 4,500 人の全員が毎月アクティブとは限らないため、低 / 中位 / 高の 3 段階（low / expected / high）で想定する。
  - 低位（保守的）シナリオ: MAU ≈ 900 人（登録の約 20%）
  - 中位（想定）シナリオ: MAU ≈ 1,575 人（登録の約 35%）— 最も現実的と考える基準線
  - 高位（楽観的）シナリオ: MAU ≈ 2,250 人（登録の約 50%）
...
- **Write Request Unit（WRU）**: 1 KB までの標準書き込み 1 回 = 1 WRU。東京（ap-northeast-1）代表値 **約 $1.4246 / 100 万 WRU**。参考: バージニア北部（us-east-1）代表値 **約 $1.25 / 100 万 WRU**（東京より安価）。
- **Read Request Unit（RRU）**: 4 KB までの結果整合性読み取り 1 回 = 0.5 RRU（強整合は 1 RRU）。東京（ap-northeast-1）代表値 **約 $0.285 / 100 万 RRU**。参考: バージニア北部（us-east-1）代表値 **約 $0.25 / 100 万 RRU**。
- **ストレージ**: 東京（ap-northeast-1）**約 $0.285 / GB-月**。参考: バージニア北部（us-east-1）**約 $0.25 / GB-月**。
- **TTL 削除**: 期限切れアイテムの TTL 削除は **無料**（日次 / 週次 / 月次バケットの自動失効に有効）。
- **DynamoDB Streams（任意）**: 有効化する場合は読み取りリクエスト（GetRecords）に課金。今回の基本構成では未使用として試算。

> リージョン差の目安: us-east-1 は東京より概ね 1〜2 割程度安価です。本試算は保守的に**東京リージョンの単価**で計算します（東京で成立すれば us-east-1 ではさらに下振れる、という読み方ができます）。
...
**中位（想定）シナリオ（MAU = 1,575 人）**

| 項目 | 計算 | 月間回数 |
|---|---|---|
| XP 獲得イベント | 1,575 人 × 20 回/日 × 15 日 | 472,500 |
| Write 回数（×4 バケット） | 472,500 × 4 | 1,890,000 WRU |
| ランキング参照 | 1,575 人 × 5 回/日 × 15 日 | 118,125 |
| Read オペレーション（×2 種別） | 118,125 × 2 | 236,250 回 |
| Read（上位100件 ≒ 25 RRU/回で概算） | 236,250 × 25 | 5,906,250 RRU |
...
単価は保守的に東京リージョン（ap-northeast-1）の代表値を用います。低 / 中位 / 高の 3 シナリオを併記します。

| 費目 | 単価（東京・代表値） | 低位（MAU 900） | 中位（MAU 1,575） | 高位（MAU 2,250） |
|---|---|---|---|---|
| Write | $1.4246 / 100万 WRU | 1.08M × 1.4246 ≈ **$1.54** | 1.89M × 1.4246 ≈ **$2.69** | 2.70M × 1.4246 ≈ **$3.85** |
| Read | $0.285 / 100万 RRU | 3.375M × 0.285 ≈ **$0.96** | 5.90625M × 0.285 ≈ **$1.68** | 8.4375M × 0.285 ≈ **$2.40** |
| ストレージ | $0.285 / GB-月 | 1GB 未満 ≈ **$0.29 未満** | 1GB 未満 ≈ **$0.29 未満** | 1GB 未満 ≈ **$0.29 未満** |
| TTL 削除 | 無料 | $0 | $0 | $0 |
| **DynamoDB 合計（概算）** | | **約 $3 / 月** | **約 $4 〜 $5 / 月** | **約 $6 〜 $7 / 月** |

> us-east-1 の単価（Write $1.25 / Read $0.25 / ストレージ $0.25）で計算すると、上記より各費目が概ね 1〜2 割安くなります（例: 中位の Write は 1.89M × 1.25 ≈ $2.36、Read は 5.90625M × 0.25 ≈ $1.48）。いずれにせよ**月額 1 桁ドル台**のオーダーに収まる見込みです。
