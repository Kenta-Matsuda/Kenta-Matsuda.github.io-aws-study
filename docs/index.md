...
- [NotebookLM 向け学習パックと学習分析 API（保存 / 他 SaaS 連携）](action-required/issue-165-notebooklm-pack-and-apis.md) — 🟡 一部対応 / 関連: #165 / PR #180 ※ クライアント側の学習パック生成（リソースリンク集 Markdown・CSV / 学習ルート Markdown / 用語集 CSV。`js/studyPack.js`）は実装済み。検索 API・分析 API・継続収集パイプラインは 🔴 未対応（要対応）
