...
### 「静的サイトだからバックエンド必須」という短絡的な断定をしない（実装可否判断の前提条件）

「このサイトは静的サイトだから、この機能にはバックエンドが必要で実現不能」という結論に反射的に飛びつかないこと。ある機能を「実現不能 / バックエンド必須」として記録する前に、必ず次を評価する。

- **ブラウザ（クライアントサイド）が、必要な LLM / リモート（ホスト型・マネージド）MCP の HTTP エンドポイントを直接呼べないかを先に検討する。** 本リポジトリの `js/gemini.js` は既に、ブラウザの `fetch()` からユーザー保管の API キーを使って `https://generativelanguage.googleapis.com` を直接呼んでいる（`js/ai.js` はプロバイダのファサード）。つまり**自前バックエンドを持たない静的サイトでも、リモートの HTTP LLM / MCP エンドポイントをブラウザから直接呼べる**ことは、このコードベース自身が実証済みである。
- したがって、実現を阻む要因を記述するときは「バックエンドが無いから」ではなく、**実際の残存ブロッカーである CORS / 認証（API キーの安全な取り扱い）/ レート制限**として具体的に挙げる。これらのどれが問題になるかを検討したうえでのみ、「クライアントサイドだけでは困難な部分」を切り出す。
- ブラウザから直接呼べる部分（クライアントサイドで前進できる分）は、それでも実装して PR を出す。バックエンドが必要と判断した残りの部分だけを `docs/action-required/` に構造化して残す。

### ネットワーク制限下（INTEGRATIONS_ONLY 等）での「実現不能」断定の禁止

外部ドキュメントや外部エンドポイントを取得できない実行環境（INTEGRATIONS_ONLY など、外部ネットワークが遮断されている場合）では、**取得できないという理由だけで「実現不能」「不可能」と断定してはならない。** 検証できないのは実現可否ではなく「外部の事実」であることを区別する。

- 検証できなかった具体的な外部事実（例: 公式 MCP エンドポイント名、必要な認証方式、CORS の許可状況など）は、**断定せず** `要確認 / 公式ドキュメントで要確認` と明記し、**留保事項（reservation）**として記録する。
- そのうえで、クライアントサイドで前進できる部分（client-side-forward-able な部分）は**留保事項とは切り離して実装・出荷する**。外部事実が未確認であることは、前進できる実装を止める理由にはならない。
- 「外部が確認できないので不可能」という過度に確信的な結論は、後続の作業を不必要に止めるため禁止する。

### 外部エンドポイントの利用可否は「推論」ではなく「実測」で決める

上記 2 節は「不可能と断定するな」というルールだが、その反動で**「リモートなら呼べるはず」と可能側に断定することも同じ誤りである**。可否はどちらの向きであっても推論で決めず、ネットワークが使える環境で実際にリクエストを投げて確定させる。

- ネットワークが使える環境では、**`Origin` ヘッダーを付けた実リクエストと `OPTIONS`（プリフライト）の両方**を実行し、`Access-Control-Allow-Origin` / `Access-Control-Expose-Headers` / プリフライトのステータスコードを根拠として記録する。レスポンスヘッダーを添えずに「ブラウザから呼べる／呼べない」と書かない。
- 実測できない環境では、前節どおり `要確認` に留める。ただし**実測できる環境に移った時点で最優先に実測し、留保を結論へ格上げする**。
- 判定に使った**再現コマンドを設計ドキュメントに残す**。相手側の設定はいずれ変わるため、次回は同じコマンドを流すだけで再判定できるようにする。
- 実例: AWS 公式のリモート MCP（`knowledge-mcp.global.api.aws` / `aws-mcp.us-east-1.api.aws`）は、実測の結果 CORS の制約でブラウザから `tools/call` できない。検証結果と再現コマンドは `docs/action-required/issue-138-ai-chat-exam-grounding.md` を参照。
