...
3. **「対応保留（skip）マーカー」による棚卸し前フィルタを適用する。** 過去に「着手不能 / 要人間対応」と判断してマーカー（後述の `agent:skipped` ラベル）を付けた issue は、原則として調査対象から除外する。ただし、マーカーを付けた**判断時点以降に新しい更新がある**ものは除外を解除し、通常どおり調査対象に戻す（判定手順は「対応保留マーカーの運用」節を参照。`scripts/issue-triage.mjs` はこの判定を `SKIP` / `RECHECK` として出力する）。除外した issue は「今回スキップ（更新なし）」として最初に一覧で示す。
   - **【恒久ブロック索引を最初に参照する（制約の追加・明確化）】** このフィルタを適用する前に、まず [`docs/wiki/blocked-issues-index.md`](../../docs/wiki/blocked-issues-index.md) を読む。同ページには、人間 / AWS 対応が本質的に必要でクライアントサイドでは前進できない**恒久ブロック中の issue**（例: #32 グローバルリーダーボード / #162 アカウント不要 issue 投稿 / #167 フィードバック分析）が、ブロック種別・理由・解消に必要な人間 / AWS 対応・成果物ポインタ付きで一覧化されている。索引に載っている issue は、**その記録日以降に新しい人間入力（本文編集・エージェント以外の新規コメント）が無い限り、内容をゼロから読み直して再トリアージしない**（`scripts/issue-triage.mjs` が `SKIP` を出すのと同じ扱い）。これは毎回同じ「AWS バックエンドが必要 → スキップ」の再導出を避けるための**追加のゲート**であり、既存の `agent:skipped` マーカー機構（ラベル + 判断コメント）を置き換えず**補完する**。記録日以降に更新があれば索引の記載に関わらず `RECHECK` として通常調査へ戻す。この索引は既存の禁止事項（AWS 操作禁止・`main` 直接 push 禁止・force push 禁止・秘密情報の非コミット）を一切緩めない。
...
**重複スキップコメントを付けない（再コメント禁止）**: マーカー（ラベル＋判断コメント）は 1 つの判断につき **1 セットで十分**です。既に `agent:skipped` ラベルと `🤖 agent:skipped` 判断コメントが付いている issue に対し、**マーカー付与後に新規の人間入力（本文編集・エージェント以外の新規コメント）が無い限り（＝棚卸しの判定が `SKIP`）、再びスキップコメントを投稿してはいけません。またラベルを付け直す必要もありません。** 同じ「見送る」という趣旨のコメントを実行のたびに重ねると、issue のコメント欄がノイズで埋まります（実際に issue #32 では複数回の実行を跨いで同種のスキップコメントが積み上がった）。再びスキップコメントを **投稿してよいのは `RECHECK`（マーカー付与後に新規の人間入力があり、再調査した結果まだ着手不能 / 要人間対応だったとき）だけ**です。この場合の新しい判断コメントは「判断時点」を更新する意味を持ちます。`scripts/issue-triage.mjs` は既存の `🤖 agent:skipped` コメント件数（`agentSkipCommentCount`）と、`SKIP` 判定 issue への再コメント不要助言（`reSkipAdvice`）を出力するので、これを機械的なチェックとして使うこと。
...
4. 判定結果に応じて分岐する:
   - **更新なし**（判断時点以降に新しい更新が無い＝`SKIP`）: 調査対象から**除外（スキップ）**する。ラベル・コメントはそのまま残すが、**新規のスキップコメントは付けない（再コメント禁止）／ラベルの付け直しもしない**。既存のラベルと 1 件の判断コメントで十分。「今回スキップ（更新なし）」として一覧に載せるだけにする。`scripts/issue-triage.mjs` の `reSkipAdvice` と `agentSkipCommentCount` で既存コメントの蓄積を確認できる。
   - **更新あり**（判断時点以降に新しい更新がある＝`RECHECK`）: 除外を解除し、通常どおり調査対象へ戻す。再調査の結果、依然として着手不能 / 要人間対応であれば**新しい判断コメントを付け直す**（＝判断時点を更新する）。この `RECHECK` のときだけ新規スキップコメントを投稿してよい。逆に対応可能になっていれば `agent:skipped` ラベルを外してから実装に進む:
     ```
