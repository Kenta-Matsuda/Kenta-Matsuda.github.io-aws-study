...
1. **【コメントファースト・事前条件（無条件・最優先）】どの issue についても、着手・見送り（スキップ）を判断する前に、必ず先にコメントを全部読む。** これは例外なく適用される事前ゲートであり、後続のどの手順よりも先に行う。
   - **推奨: この判定は `scripts/issue-triage.mjs` で一括実行する**（主要機能 3 を参照）。同スクリプトは下記 (i)(ii)(iii) の取得と判定を決定論的に行い、要約だけを出力するため、生 JSON を読むより桁違いに低コストで、かつ見落としが起きない。
     ...
     ```
     `NODE_OPTIONS` が存在しない preload を指している環境では前置きが必要（「実行環境の事実」節を参照）。オープン PR 監査を含むため**完了に 1〜3 分かかる**。タイムアウトの短い同期実行ではなく、バックグラウンド実行 + ファイル出力にすること。**さらに、観測シェルがタイムアウトで打ち切られても子ジョブが道連れで死なないよう、親シェルから切り離して（detached で）起動し、出力はリポジトリ外に書く**（bash 例: `nohup node scripts/issue-triage.mjs > /var/tmp/asn-scratch/triage.out 2>&1 &`、または `setsid ...`）。起動シェルはすぐ戻し、**別の短いシェル呼び出しで完了（末尾の完了行 / サイズが増えなくなったか）をポーリング**する。`&` だけの起動や短時間タイムアウトのシェルでの待機は、途中で殺されて**空 / 書きかけの出力ファイル**を残すため避ける（空出力を「issue 0 件」と誤読しない。詳細は playbook の落とし穴 A-16）。
