...
- **(a) コメントを全列挙する。** ディスカッション（`/issues/{pr_number}/comments`）・インライン（`/pulls/{pr_number}/comments`）・レビュー（`/pulls/{pr_number}/reviews`）の 3 種すべてを取得し、メンテナ由来の指摘・修正依頼・追加要望を漏れなく洗い出す（`🤖 対応済み` で始まる自分の対応コメントは基準時刻の判定にのみ使い、指摘としては数えない）。
